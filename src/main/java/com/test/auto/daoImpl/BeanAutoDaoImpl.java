...
import com.test.auto.dao.BeanAutoDao;
...
public class BeanAutoDaoImpl implements BeanAutoDao {
    ...
    public boolean createBean() {
        for(TableStruct tableStruct:list){
            System.out.println(tableStruct.getTableName()+" "+tableStruct.getColumns().size());
        }
//获得配置文件的参数
//项目路径
...
//(实体类）文件内容
                ...
                String zhujie="@Entity(name='"+list.get(i).getTableName()+"')"+"\n";
...
//生成属性
                    if(j==0){
                        String idzhujie="\t"+"@Id"+"\n";
                        classCon.append(idzhujie+"\t"+"private"+" "+dateType+" "+columnName+";\n");
                    }else {
